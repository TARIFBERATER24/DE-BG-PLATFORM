...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ error?: string; detail?: string; reset?: string }> }) {
  const { error, detail, reset } = await searchParams;
  const errorText = error ? (detail || "Не успяхме да те впишем. Провери имейла и паролата.") : null;
  return (
    <div className="mx-auto max-w-md px-6 py-16">
      ...
      {reset === "success" ? <p className="mt-5 rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-800">Паролата ти беше променена. Влез с новата си парола.</p> : null}
      ...
      <form action={loginAction} className="mt-7 space-y-4 rounded-2xl border border-line bg-white p-6 shadow-sm">
        ...
        <p className="text-center text-sm text-ink-muted"><Link href="/mein-deutschland/forgot-password" className="font-semibold text-brand">Забравена парола?</Link></p>
      </form>
